...
def monitor_liquidity(symbols):
    ...
    if alerts:
        ...
        for sym, info, imb_cross, spr_cross in alerts:
            msg = f"\U0001f4ca *\u0633\u064a\u0648\u0644\u0629 {fmt_symbol(sym)}*\n"
...
def api_send_telegram():
    ...
    msg = (
        f"📊 *توقع العملة: {fmt_symbol(selected['symbol'])}*\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
...
def api_market_data_ds(symbol):
    try:
        from crypto_market_data import get_aggregated_sentiment
...
def api_backtest_ds():
    try:
        ...
        from backtest.multi_asset_backtester import quick_backtest
        from predictor import predict_price_movement
...
# ═══════════════════════════════════════════════════════════════════
# PAPER TRADING, ON-CHAIN, MULTI-EXCHANGE ENDPOINTS
# ═══════════════════════════════════════════════════════════════════
...
def api_paper_trades():
    from paper_trading import paper_engine
    return jsonify(paper_engine.get_recent_trades(50))
...
def api_paper_open():
    ...
    trade = paper_engine.open_trade(
        ...
        tf_hours=int(data.get("tf_hours", 24)),
...
def api_paper_auto():
    ...
    from paper_trading import paper_engine
    from predictor import predict_price_movement
    pred = predict_price_movement(symbol, 24)
    if "error" in pred:
        return jsonify({"error": pred["error"]}), 400
    pct = pred.get("predicted_change_pct", 0)
    conf = pred.get("confidence", 50)
    risk = pred.get("risk", {})
    sl = risk.get("suggested_stop_loss_pct", 2)
    tp = risk.get("suggested_take_profit_pct", 4)
    size = risk.get("suggested_position_size_pct", 10) / 100
    side = "LONG" if pct > 0 else "SHORT"
    trade = paper_engine.open_trade(
        symbol=symbol, side=side,
        entry_price=pred.get("current_price", 0),
        size_pct=size, stop_loss_pct=sl, take_profit_pct=tp,
        tf_hours=24, confidence=conf,
        reason=f"auto_{pred.get('grade', 'signal')}"
    )
    return jsonify({"success": True, "side": side, "predicted_change": pct, "trade_id": trade.id})

@app.route('/api/paper/reset', methods=['POST'])
def api_paper_reset():
    from paper_trading import paper_engine
    capital = request.json.get("capital", 10000) if request.json else 10000
    paper_engine.reset(float(capital))
    return jsonify({"success": True, "capital": capital})

@app.route('/api/onchain')
def api_onchain():
    from onchain_data import get_aggregated_onchain_signal, get_btc_dominance_and_metrics, get_mempool_fees
    return jsonify(get_aggregated_onchain_signal("BTC"))

@app.route('/api/onchain/global')
def api_onchain_global():
    from onchain_data import get_btc_dominance_and_metrics, get_mempool_fees
    return jsonify({"dominance": get_btc_dominance_and_metrics(), "mempool": get_mempool_fees()})

@app.route('/api/multi_exchange/<symbol>')
def api_multi_exchange(symbol):
    from multi_exchange import get_exchange_price, get_aggregated_klines
    sym = symbol.upper()
    if not sym.endswith("USDT"):
        sym += "USDT"
    price = get_exchange_price(sym)
    klines = get_aggregated_klines(sym, "1h", 10)
    fmt = [{"close": k["close"], "exchange": k["exchange"]} for k in klines[:5]] if klines else []
    return jsonify({"price_data": price, "sample_klines": fmt})
