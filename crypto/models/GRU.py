...
from keras.layers import Dense, GRU, Bidirectional, BatchNormalization, Dropout
...
class MyGRU:
    def __init__(self, args):
        ...
        self.num_layers = getattr(args, 'num_gru_layers', 3)
        ...
        for i in range(self.num_layers):
            ...
            if i == 0:
                if self.use_bidirectional:
                    self.model.add(Bidirectional(GRU(self.hidden_dim, return_sequences=return_seq), input_shape=(1, shape_)))
                else:
                    self.model.add(GRU(self.hidden_dim, return_sequences=return_seq, input_shape=(1, shape_)))
            else:
                if self.use_bidirectional:
                    self.model.add(Bidirectional(GRU(self.hidden_dim, return_sequences=return_seq)))
                else:
                    self.model.add(GRU(self.hidden_dim, return_sequences=return_seq))
